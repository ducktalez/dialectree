...
- **zickzack.html** – API-backed zig-zag view with chronological layout, interactive input (add arguments, vote, comment), served at `/` and `/zickzack`. Stage 0 includes SRT import modal (paste .srt → parse → store as transcript). Fan/Fächer mode is implemented but commented out (`# TODO: post-dev`). View state (selected topic + stage) is reflected in `location.hash` (`#topic=<id>&stage=<n>`) so reload and back/forward restore the view. **Multi-Node-Muster** (Gish Gallop, Creeping Relativization, Other) are surfaced as a second overlay layer alongside polygon groups: a "🧩 Muster markieren" button in the legend opens a selection mode; selected cards plus name + `PatternType` are persisted via `POST /api/patterns/`. Patterns render as dashed convex-hull outlines colored by `pattern_type` (red = Gish Gallop, orange = Creeping Relativization, neutral grey = Other) and can be deleted from the legend's "Muster" section. **Definition Forks** (Z.5 preview): every Stage-3+ card carries a `⑂`-badge showing the count of competing term interpretations; clicking opens an inline panel with the existing forks (term + variant + optional description) plus an add-row. Each list row has a ✏-button that switches the row to an inline edit form (single-row edit; calls `PATCH /api/definition-forks/{id}`). Forks are loaded once per topic via `GET /api/definition-forks/?topic_id=…` and mutated via `POST` / `PATCH` / `DELETE /api/definition-forks/{id}`.
- **rauchen.html** – archived snapshot (route removed).
- **quellen.html** – pr0gramm-style "Quellensammlung" served at `/quellen`. **Fixed 128×128 px tiles, 8 per row, 4 px gap → page width 1052 px** (auto-wrap below 1100 px). Click expands detail inline at end of row (`.detail-row` spans full grid width). Detail layout below the media (mirrors pr0gramm): **header row** = vote widget (left) + title; **submeta line** = kind · date · uploader · source URL (tiny, 0.62 rem); **tags row** = top 5 visible, "+N weitere anzeigen…" reveals the rest; **description** (optional, blue-bar block); **comments**; **usages** ("Verwendung in Diskussionen"). Inline media player for VIDEO/AUDIO — YouTube/Vimeo embed or native `<video>`/`<audio>` for uploaded files. Inline comment + usage forms; "Neue Quelle" modal with multipart upload (`POST /api/sources/`, accepts both `thumbnail` and `media` files). Per-tile ▲/▼ voting widget (own vote remembered client-side in `localStorage` under `quellen.votes`; server has no auth yet and trusts the `previous` value sent by the client). Filterable by kind, tags (incl. `TOPIC:<SLUG>` namespace), full-text search; sort by `neu`/`alt`/`titel`/`top`/`kontrovers`/`zufall`; URL hash carries view state (`#id=<n>&tag=…&q=…`). Backed by `routers/sources.py` with SQLAlchemy models `Source` / `SourceTag` (n:m) / `SourceComment` / `SourceUsage` (`SourceUsage.argument_id` is a real FK with `ON DELETE SET NULL`). Legacy `backend/app/data/sources.json` is still used as a one-shot seed source on first start. Thumbnails live in `backend/app/static/sources/<id>.{svg,png,jpg,…}`, uploaded media under `…/sources/media/<id>.<ext>`; auto-generated SVG placeholder if no thumbnail uploaded. Lightweight duplicate detection via `GET /api/sources/similar?title=…&description=…` (token-set Jaccard, no embeddings) — the "Neue Quelle" modal shows live warnings about possible duplicates as the user types. Stand-in for future embedding-based dedup.
...
| **Source** | `sources` | `title`, `kind`, `url`, `thumbnail`, `media_url`, `up`, `down` | Quellensammlung entry. Per-user vote tracking deferred |
| **SourceTag** | `source_tags` (+ `source_tag_link` n:m) | `name` (unique) | Free-form tag namespace separate from `Tag` (uses `QUELLE`/`TOPIC:<SLUG>` conventions) |
| **SourceComment** | `source_comments` | `source_id`, `user` (display name), `text` | Comments on sources (no auth yet → display-name only) |
| **SourceUsage** | `source_usages` | `source_id`, `argument_id` (FK, ON DELETE SET NULL), `context`, `note` | Tracks where a source has been cited in argument nodes |
...
| `SourceKind` | QUOTE, VIDEO, AUDIO, PAPER, TWEET, IMAGE, TEXT | Source.kind |
