...
- **quellen.html** – pr0gramm-style "Quellensammlung" served at `/quellen`. Fixed 8-column grid (responsive 6/4/3) of thumbnails; click expands detail inline at end of row (`.detail-row` spans full grid width). Detail shows large preview (inline media player for VIDEO/AUDIO — YouTube/Vimeo embed or native `<video>`/`<audio>` for uploaded files), tags (prominent), title, description, source URL, usages, comments. Inline comment + usage forms; "Neue Quelle" modal with multipart upload (`POST /api/sources/`, accepts both `thumbnail` and `media` files). Per-tile ▲/▼ voting widget (own vote remembered client-side in `localStorage` under `quellen.votes`; server has no auth yet and trusts the `previous` value sent by the client). Filterable by kind, tags (incl. `TOPIC:<SLUG>` namespace), full-text search; sort by `neu`/`alt`/`titel`/`top`/`kontrovers`; URL hash carries view state (`#id=<n>&tag=…&q=…`). Backed by `routers/sources.py` reading from `backend/app/data/sources.json` (no DB table yet — Phase 2 promotes to model). Thumbnails live in `backend/app/static/sources/<id>.{svg,png,jpg,…}`, uploaded media under `…/sources/media/<id>.<ext>`; auto-generated SVG placeholder if no thumbnail uploaded.
